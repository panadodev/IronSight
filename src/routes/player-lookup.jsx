...
const Route = createFileRoute("/player-lookup")({
  ...
  validateSearch: (s) => ({
    steam: typeof s.steam === "string" && /^\d{17}$/.test(s.steam) ? s.steam : void 0
  }),
...
function PlayerLookupPage() {
  ...
  const playerTickets = useMemo(() => {
    ...
    return TICKETS.filter(
      (t) => t.type === "player_report" ? t.subjectId === subject.steamId : t.reporterId === subject.steamId
    );
  }, [subject]);
  ...
  >
            ...
            {input.trim().length > 0 && !/^\d{17}$/.test(input.trim()) && <p className="mt-2 text-[11px] text-warning">
                Steam IDs are 17 digits.
              </p>}
          </div>
        </div>

        {!subject || !stats ? <div className="flex-1 grid place-items-center text-muted-foreground text-sm">
                  ...
                  <div className="flex items-center justify-between gap-4 mb-5">
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h2 className="text-lg font-semibold truncate">{subject.name}</h2>
  ...
  >
                      ...
                      {!isSupportOnly && <div className="relative">
                          <button
  ...
  />
                              <div className="absolute right-0 top-full mt-1 z-50 w-44 bg-background ring-1 ring-border rounded-md shadow-lg p-1">
                                ...
                                {REPORT_CATEGORIES.map((c) => <button
    key={c.id}
    onClick={() => {
  ...
  >
                  ...
                  {!isSupportOnly && <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6">
  ...
  />
    ...
    /* Tickets — quick hyperlinks to every related ticket */
              ...
              <PlayerTicketsSection subjectId={subject.steamId} tickets={playerTickets} />

              {
    /* Previous Offenses — support only sees mutes */
  }
              {(() => {
    const visible = isSupportOnly ? stats.offenses.filter((o) => o.type === "Mute") : stats.offenses;
    return <section>
                    ...
                    {visible.length === 0 ? <p className="text-xs text-muted-foreground italic">No prior offenses.</p> : <OffensesTable offenses={visible} />}
                  </section>;
  })()}
    ...
    /* Bans on other orgs — read-only, shared from partner orgs' banlists */
              ...
              {!isSupportOnly && <ExternalBansSection subjectId={subject.steamId} />}
    ...
    /* Linked accounts — IP-overlap intel. Hidden from support, IPs masked for admins. */
              ...
              {!isSupportOnly && <LinkedAccountsSection
  ...
  />}
    ...
    /* Hit % by Distance */
              ...
              {!isSupportOnly && <HitDistanceSection subjectId={subject.steamId} />}

              {
    /* Current team + Previous team + Associated players — 1x3 horizontal row */
  }
              {!isSupportOnly && <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <TeammatesSection subjectId={subject.steamId} category="teaming" only="current" />
                  <TeammatesSection subjectId={subject.steamId} category="teaming" only="previous" />
                  <FriendlyRecipientsSection subjectId={subject.steamId} serverId={null} />
                </div>}
              {isSupportOnly && <FriendlyRecipientsSection subjectId={subject.steamId} serverId={null} />}

              {
    /* Server History */
  }
              {!isSupportOnly && <ServerHistorySection
    subjectId={subject.steamId}
...
function AlertBrowser({ entries }) {
  ...
  const aged = useMemo(() => {
    return entries.map((e) => {
      const when = e.msg.occurredAt ? Date.parse(e.msg.occurredAt) : Date.parse(e.ticket.createdAt);
      ...
      <h3 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-3 flex items-center justify-between">
        <span className="flex items-center gap-3">
          <span className="flex items-center gap-1.5">
            <span className="size-2 rounded-full" style={{ background: F7_COLOR }} />
            <span style={{ color: F7_COLOR }}>F7</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="size-2 rounded-full" style={{ background: TH_COLOR }} />
            <span style={{ color: TH_COLOR }}>Thorium</span>
          </span>
        </span>
        <span className="font-mono normal-case tracking-normal text-muted-foreground">
          {f7Visible > 0 && <span className="mr-2" style={{ color: F7_COLOR }}>
              {f7Visible} F7
            </span>}
          {thVisible > 0 && <span style={{ color: TH_COLOR }}>{thVisible} Thorium</span>}
  ...
  />
      ...
      <div className="relative rounded-lg ring-1 ring-border/50 bg-surface/30 p-2">
        {visible.length === 0 ? <div className="px-1 py-6 text-xs text-muted-foreground italic text-center">
            No F7 reports or Thorium alerts in the last {dayWindow} day{dayWindow === 1 ? "" : "s"}.
          </div> : <svg
    viewBox={`0 0 ${W} ${H}`}
  ...
  />
    ...
    return <g key={`${d}-${i}`} className="text-muted-foreground">
                  <line
      x1={x}
      x2={x}
      y1={PAD_T + innerH}
    ...
    >
      ...
      /* glow ring on active */
                    ...
                    {isActive && <circle cx={0} cy={headCy} r={headR + 5} fill={fill} opacity={0.18} />}
                    {
      /* triangle tail */
    }
                    <path
  ...
  />
              ...
              <div className="flex items-center gap-2">
                {pinnedIdx !== null && <span className="text-[9px] font-mono uppercase tracking-widest text-muted-foreground">
                    pinned
                  </span>}
                <button
    onClick={() => {
  ...
  >
            ...
            <div className="px-3 py-1.5 border-t border-border/60 text-[10px] font-mono text-muted-foreground">
              {active.entry.msg.authorName} · {active.entry.msg.occurredAt ? new Date(active.entry.msg.occurredAt).toLocaleString() : active.entry.msg.timestamp}
              {" \xB7 "}
              {active.ageDays < 1 ? "today" : `${Math.round(active.ageDays)}d ago`}
            </div>
          </div>}
      </div>
    </section>;
...
function PlayerTicketsSection({
  ...
  return <section>
      ...
      {tickets.length === 0 ? <p className="text-xs text-muted-foreground italic">No tickets involving this player.</p> : <div className="space-y-3">
          {["active", "waiting", "closed"].map(
    (g) => groups[g].length === 0 ? null : <TicketGroup key={g} label={g} subjectId={subjectId} tickets={groups[g]} />
  )}
        </div>}
    </section>;
}
function TicketGroup({
  ...
  const labelTone = label === "active" ? "text-success" : label === "waiting" ? "text-warning" : "text-muted-foreground";
  return <div className="bg-surface/40 ring-1 ring-border rounded-lg overflow-hidden">
      <div className="px-3 py-1.5 border-b border-border/60 flex items-center justify-between">
        <span className={`text-[10px] font-mono uppercase tracking-widest ${labelTone}`}>
          {label}
        </span>
        <span className="text-[10px] font-mono text-muted-foreground">{tickets.length}</span>
      </div>
