...
const Route = createFileRoute("/submit")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in with Steam and report cheaters, appeal bans, or get support."
      }
    ]
  }),
...
function SubmitPage() {
          ...
          <div className="max-w-xl mx-auto p-8">
            <div className="rounded-lg ring-1 ring-border bg-surface/40 p-6 space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                The selected organization is not available for your current session.
              </p>
  ...
  const associatedPlayers = associatedIds.map((id) => getPlayer(id)).filter((p) => Boolean(p));
  const canAddAssociated = category === "teaming" && associatedIds.length < MAX_ASSOCIATED;
  ...
  const existingCase = useMemo(() => {
    if (type !== "player_report" || !subjectId) return null;
    return TICKETS.find(
      (t) => t.type === "player_report" && t.subjectId === subjectId && t.category === category && t.status !== "banned"
    ) ?? null;
        ...
        <div className="flex-1 grid place-items-center p-6">
          <div className="w-full max-w-md bg-surface/60 ring-1 ring-border rounded-xl p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              We use your Steam profile to verify your in-game activity and prior history.
            </p>
            <button
        ...
        <div className="flex-1 grid place-items-center p-6">
          <div className="w-full max-w-md bg-surface/60 ring-1 ring-border rounded-xl p-8 text-center">
            ...
            <h1 className="text-xl font-semibold mb-1">
              {submitted.aggregated ? "Added to existing case" : "Ticket submitted"}
            </h1>
            <p className="text-sm text-muted-foreground mb-2">
              {submitted.aggregated ? "Your evidence was attached to the open case " : "Your ticket reference is "}
              <span className="font-mono text-brand">{submitted.ref}</span>.
            </p>
            <p className="text-xs text-muted-foreground mb-6">
              Routed to <span className="text-foreground">{routedTeam ? TEAM_META[routedTeam].label : ""}</span>.
              {type === "player_report" ? <>
                  {" "}Track status in{" "}
  ...
  return <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto p-8 space-y-8">
          <header>
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-prose">
              Pick a ticket type. Reports require you to choose the server you saw the player on,
              then pick them from the live roster.
            </p>
    ...
    return <button
      ...
      className={"text-left p-4 rounded-lg ring-1 transition-colors " + (active ? "bg-brand/10 ring-brand/30" : "bg-surface/40 ring-border hover:bg-surface/70")}
    >
                    <div className="flex items-center justify-between mb-1">
                      <p
      className={"text-sm font-semibold " + (active ? "text-brand" : "text-foreground")}
    >
                        {t.label}
                      </p>
    ...
    /* Player report: progressive disclosure — only show next step when previous one is satisfied */
          ...
          {type === "player_report" && <section className="space-y-4">
    ...
    return <button
      ...
      className={"text-left px-4 py-3 rounded-lg ring-1 transition-colors flex items-center justify-between " + (active ? "bg-brand/10 ring-brand/30" : "bg-surface/40 ring-border hover:bg-surface/70")}
    >
                        <div>
                          <p
      className={"text-sm font-semibold " + (active ? "text-brand" : "text-foreground")}
    >
              ...
              {serverId && <div className="space-y-3">
                  <label className="block text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
                    Step 2 · Reported player ({players.length} online on this server)
                  </label>
                  <PlayerCombobox
    players={players}
    value={subjectId}
  ...
  />
                  {subject && subjectId && <div className="p-4 bg-background ring-1 ring-border rounded-md flex items-center gap-4">
                      <div
  ...
  >
                        {subject.name.replace(/[\[\]]/g, "").slice(0, 2).toUpperCase()}
                      </div>
              ...
              {subjectId && <div className="space-y-3">
                  <label className="block text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
    ...
    return <button
      ...
      className={"text-left p-3 rounded-lg ring-1 transition-colors " + (active ? "bg-brand/10 ring-brand/30" : "bg-surface/40 ring-border hover:bg-surface/70")}
    >
                          <p
      className={"text-sm font-semibold " + (active ? "text-brand" : "text-foreground")}
    >
                  ...
                  <p className="text-[10px] font-mono text-muted-foreground">
                    Each rule violation is its own ticket. Reporting the same player for cheating
                    AND teaming creates two separate cases.
                  </p>
                </div>}
            </section>}
    ...
    /* Title — non-report tickets only */
          ...
          {type && type !== "player_report" && <section className="space-y-3">
              <label className="block text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
                Title
              </label>
              <input
  ...
  />
          ...
          {type === "player_report" ? subjectId && category && <>
                {category === "teaming" && <section className="space-y-3">
                    <label className="flex items-center justify-between text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
                      ...
                      <span className="text-muted-foreground/70 normal-case tracking-normal font-mono">
                        up to {MAX_ASSOCIATED} · {associatedIds.length}/{MAX_ASSOCIATED}
                      </span>
                    </label>
                    <p className="text-[11px] text-muted-foreground">
                      Add anyone you saw teaming with the reported player. Each one you add
                      is filed as being associated with this player and counted on their
                      teaming history.
                    </p>
                    {associatedPlayers.length > 0 && <ul className="space-y-1.5">
                        {associatedPlayers.map((p) => <li
    key={p.steamId}
  ...
  >
                            ...
                            <button
    onClick={() => setAssociatedIds(
      (ids) => ids.filter((id) => id !== p.steamId)
    )}
  ...
  >
    ...
    onChange={(id) => {
      if (!id) return;
      setAssociatedIds(
        (ids) => ids.includes(id) || ids.length >= MAX_ASSOCIATED ? ids : [...ids, id]
      );
    }}
    placeholder={associatedIds.length === 0 ? "Add an associated player (optional)..." : `Add another${associatedIds.length >= MAX_ASSOCIATED - 1 ? " (last one)" : ""}...`}
  />
                      </div>}
                    {!canAddAssociated && associatedIds.length >= MAX_ASSOCIATED && <p className="text-[10px] font-mono text-muted-foreground">
  ...
  />
                ...
                <section className="space-y-3">
                  <label className="flex items-center justify-between text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
                    <span>Step 5 · Evidence (links to clips, screenshots, recordings)</span>
                    <span className="text-muted-foreground/70 normal-case tracking-normal font-mono">
  ...
  />
                  <p className="text-[10px] text-muted-foreground">
                    Reports without evidence can be hidden by staff when reviewing — only your name
                    stays visible in the reporter list.
                  </p>
                </section>
              </> : type ? title.trim() && <section className="space-y-3">
                <label className="block text-[10px] uppercase font-bold text-muted-foreground tracking-widest">
                  Details
                </label>
                <textarea
  ...
  />
          ...
          {type && <div className="flex justify-center pt-2">
              <button
    onClick={submit}
    disabled={type === "player_report" ? !description.trim() || !subjectId : !body.trim() || !title.trim()}
