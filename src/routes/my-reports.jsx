...
const Route = createFileRoute("/my-reports")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Chat with staff on your tickets and track the status of players you've reported."
      }
    ]
  }),
...
const STATUS_BLURB = {
  pending: "Staff are actively investigating. We'll update you when there's a verdict.",
  case_closed: "We're no longer actively investigating this report. If you have more proof, send it and we'll reopen your case.",
  banned: "We banned the player you reported. Thanks for keeping the server clean."
};
...
function MyTicketsPage() {
    ...
    for (const t of scoped) {
      ...
      for (const r of t.reports) {
        if (r.reporterId === CURRENT_USER_ID) {
          out.push({ ticketId: t.id, number: t.number, subjectId: t.subjectId, category: t.category, entry: r });
        }
      }
    }
    return out.sort((a, b) => b.entry.submittedAt.localeCompare(a.entry.submittedAt));
  }, [scoped]);
    ...
    setTickets(
      (all) => all.map((t) => {
        ...
        return {
          ...
          reports: t.reports.map(
            (r) => r.id === row.entry.id ? {
              ...r,
              status: r.status === "banned" ? "banned" : "pending",
              evidence: r.evidence.trim().length > 0 ? r.evidence + "\n\u2014 Update \u2014\n" + draft.trim() : draft.trim(),
  ...
  const sendReply = (ticketId) => {
    ...
    setTickets(
      (all) => all.map((t) => t.id === ticketId ? { ...t, messages: [...t.messages, msg] } : t)
    );
  ...
  return <div className="min-h-screen flex flex-col bg-background text-foreground">
    ...
    const active = tab === id;
    const count = id === "tickets" ? myTickets.length : reportRows.length;
    return <button
      key={id}
      onClick={() => setTab(id)}
      className={"px-4 py-2.5 text-xs font-semibold uppercase tracking-wider border-b-2 -mb-px transition-colors " + (active ? "border-brand text-brand" : "border-transparent text-muted-foreground hover:text-foreground")}
    >
                  {id === "tickets" ? "Tickets" : "Player Reports"}
                  <span className="ml-2 text-[10px] font-mono opacity-70">{count}</span>
                </button>;
  })}
          </div>

          {tab === "tickets" && (myTickets.length === 0 ? <div className="bg-surface/40 ring-1 ring-border rounded-lg p-8 text-center">
    ...
    >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">
                            {TICKET_TYPE_LABEL[t.type]} · opened {t.createdLabel}
                          </p>
                        </div>
                        <span
      className={"text-[10px] font-bold uppercase tracking-wider ring-1 rounded px-2 py-0.5 " + (closed ? "text-muted-foreground ring-border bg-surface" : "text-success ring-success/30 bg-success/10")}
    >
                          {closed ? "Closed" : "Active"}
                        </span>
                      </button>
                      {open && <div className="px-4 pb-4 space-y-3 border-t border-border bg-background/40">
                          <div className="space-y-2 pt-4">
                            {t.messages.length === 0 && <p className="text-xs italic text-muted-foreground">
                                No replies yet — staff will respond here.
                              </p>}
                            {t.messages.map((m, i) => {
      const mine = m.authorKind === "reporter" && m.authorId === CURRENT_USER_ID;
      return <div
        key={i}
        className={"rounded-md p-3 ring-1 " + (m.authorKind === "system" ? "bg-surface/40 ring-border text-muted-foreground" : mine ? "bg-brand/10 ring-brand/20" : "bg-surface/60 ring-border")}
      >
                                  <div className="flex items-center justify-between mb-1">
                                    <p className="text-[10px] font-mono uppercase tracking-wider">
                                      <span
        className={m.authorKind === "staff" ? "text-brand" : mine ? "text-foreground" : "text-muted-foreground"}
      >
                                        {m.authorName}
                                      </span>
                                      {m.authorKind === "staff" && <span className="ml-2 text-muted-foreground">Staff</span>}
                                    </p>
                                    <span className="text-[10px] font-mono text-muted-foreground">
                                      {m.timestamp}
                                    </span>
    ...
    >
          ...
          {tab === "reports" && <>
              <p className="text-xs text-muted-foreground -mt-2">
                Reports never open a direct chat — but you can attach more proof to any pending
                or closed case.
              </p>
              {reportRows.length === 0 ? <div className="bg-surface/40 ring-1 ring-border rounded-lg p-8 text-center">
                  <p className="text-sm text-muted-foreground mb-4">
  ...
  >
    ...
    >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {row.category && <span className="text-[10px] font-mono uppercase tracking-wider ring-1 rounded px-1.5 py-0.5 bg-surface ring-border text-muted-foreground">
    ...
    >
                        ...
                        {open && <div className="px-4 pb-4 space-y-4 border-t border-border bg-background/40">
                            ...
                            <div>
                              ...
                              {row.entry.evidence.trim().length > 0 ? <p className="text-sm whitespace-pre-line font-mono break-all text-foreground/90 leading-relaxed">
                                  {row.entry.evidence}
                                </p> : <p className="text-xs text-muted-foreground italic">
                                  You didn't attach evidence. Staff are more likely to act when you add proof.
                                </p>}
                            </div>
                            {row.entry.status !== "banned" && <div className="space-y-2">
