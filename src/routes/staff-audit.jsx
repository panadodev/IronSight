...
function buildAudit(staffId) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (needsTarget) {
      ...
      entry.target = {
        name:
          pick(mulberry(nameSeed), NAMES_POOL) + String(randInt(rng, 1, 999)),
    ...
    if (type === "ban")
      entry.note = pick(rng, [
        "Cheating \xB7 permanent",
        "Toxicity \xB7 30d",
        "Teaming \xB7 permanent",
        "Ban evasion \xB7 permanent",
      ]);
    if (type === "mute")
      entry.note = pick(rng, [
        "Toxicity \xB7 24h",
...
function ActivityChart({ days, enabled }) {
  const w = 800;
  const h = 260;
  const padL = 36;
  const padR = 12;
  ...
  const max = Math.max(
    1,
    ...days.flatMap((d) =>
      ACTIVITY_TYPES.filter((t) => enabled.has(t.key)).map((t) => d[t.key]),
    ),
  );
...
function StaffAuditPage() {
  ...
  const totals = useMemo(() => {
    ...
    for (const d of days)
      for (const t of ACTIVITY_TYPES) out[t.key] += d[t.key];
    return out;
  }, [days]);
  return (
                    ...
                    <table className="w-full text-[11px] font-mono">
                      <thead>
                        <tr className="text-[9px] uppercase tracking-wider text-muted-foreground bg-surface/60">
                          <th className="px-2 py-1.5 text-left font-medium w-20">
                            When
                          </th>
                      ...
                      <tbody>
                        {filteredEntries.map((e) => {
                          const t = ACTIVITY_TYPES.find(
                            (x) => x.key === e.type,
                          );
                          return (
                            <tr key={e.id} className="border-t border-border">
                              <td className="px-2 py-1 text-muted-foreground whitespace-nowrap">
                                {e.daysAgo === 0 ? "Today" : `-${e.daysAgo}d`} ·{" "}
                                {e.time}
                              </td>
