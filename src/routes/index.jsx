...
import { AppealSidebar, AppealModerationActions } from "@/components/appeal-sidebar";
...
const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Rust server staff support console: triage tickets, assign teams, audit players."
      }
    ]
  }),
...
function StaffDashboard() {
  const { view, activeStaff, activeRank, rankOf, staff, selectedOrgIds, isOwner } = useAuth();
  void activeRank;
  ...
  const [selectedId, setSelectedId] = useState(
    ticketSearchId && TICKETS.some((t) => t.id === ticketSearchId) ? ticketSearchId : TICKETS[0].id
  );
  useEffect(() => {
        ...
        <div className="flex-1 grid place-items-center p-6">
          <div className="text-center max-w-md">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              You're in <span className="text-foreground">PUBLIC</span> view. Switch to{" "}
              <span className="text-foreground">STAFF</span> view in the top bar, or submit a
              ticket via the Player Portal.
            </p>
            <Link
      to="/submit"
  ...
  const canSee = (t) => {
    ...
    const minRank = t.type === "player_report" && t.category === "toxicity" ? 1 : TYPE_MIN_RANK[t.type];
  ...
  const visible = useMemo(() => {
    ...
    const filtered = tickets.filter(canSee).filter((t) => selectedOrgIds.includes(orgForTicket(t.number))).filter((t) => {
      if (searching) return true;
      if (queueView === "closed") return CLOSED_STATUSES.includes(t.status);
      if (queueView === "waiting") return t.status === "waiting_response";
      return !CLOSED_STATUSES.includes(t.status) && t.status !== "waiting_response";
    }).filter((t) => typeFilter === "all" ? true : t.type === typeFilter).filter((t) => teamFilter === "all" ? true : t.team === teamFilter).filter(ticketMatchesQuery);
    ...
    return sorted;
  }, [tickets, typeFilter, teamFilter, sortBy, queueView, activeRank, isOwner, selectedOrgIds, playerQuery]);
  const selected = tickets.find((t) => t.id === selectedId && canSee(t)) ?? visible[0] ?? null;
  ...
  const updateTicket = (id, patch) => setTickets(
    (all) => all.map((t) => {
      if (t.id !== id) return t;
      const next = { ...t, ...patch };
      ...
      if (patch.assigneeId !== void 0 && patch.assigneeId !== t.assigneeId) {
        const who = patch.assigneeId ? staff.find((s) => s.id === patch.assigneeId)?.name ?? "unknown" : null;
        logs.push(who ? `assigned ticket to ${who}` : "unassigned the ticket");
      }
      if (patch.team !== void 0 && patch.team !== t.team) {
        logs.push(`reassigned ticket to ${TEAM_META[patch.team].label}`);
      }
      if (patch.restrictedRank !== void 0 && patch.restrictedRank !== t.restrictedRank) {
        logs.push(
          patch.restrictedRank === null ? "unlocked ticket visibility" : `locked ticket to rank \u2265 ${patch.restrictedRank}`
        );
      }
  ...
  useEffect(() => {
    setTickets(
      (all) => all.map((t) => {
        if (t.type === "player_report" && t.status === "waiting_response" && t.subjectId && getPlayer(t.subjectId).lastSeen.startsWith("Now")) {
  ...
  const assignToMe = () => selected && activeStaff && updateTicket(selected.id, {
  ...
  const markCleared = () => {
    ...
    updateTicket(selected.id, {
      status: "cleared",
      reports: selected.reports.map(
        (r) => r.status === "pending" ? { ...r, status: "case_closed" } : r
      )
    });
  };
  const reopenReport = () => {
    ...
    updateTicket(selected.id, {
      status: "open",
      reports: selected.reports.map(
        (r) => r.status === "case_closed" ? { ...r, status: "pending" } : r
      )
    });
  };
  ...
  const toggleHide = () => {
    if (!selected) return;
    if (selected.restrictedRank !== null && selected.restrictedRank <= selectedOrgRank) {
  ...
  const sendMessage = () => {
    ...
    const nextStatus = !isNote && waitForResponse ? "waiting_response" : selected.status === "open" ? "in_progress" : selected.status;
    updateTicket(selected.id, {
      status: nextStatus,
      messages: [
        ...selected.messages,
        {
          authorId: activeStaff.id,
          authorName: isNote ? `${activeStaff.name} (internal)` : activeStaff.name,
  ...
  const togglePin = (index) => {
    if (!selected) return;
    updateTicket(selected.id, {
      messages: selected.messages.map(
        (m, i) => i === index ? { ...m, pinned: !m.pinned } : m
      )
    });
  };
  const canHide = selected !== null && (isOwner || (selected.restrictedRank === null ? selectedOrgRank >= TEAM_META[selected.team].rank : selectedOrgRank >= selected.restrictedRank));
          ...
          <div className="p-4 border-b border-border space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
                {queueView === "closed" ? "Closed Tickets" : queueView === "waiting" ? "Waiting Response" : "Active Queue"}
              </h2>
            ...
            <div className="flex rounded ring-1 ring-border overflow-hidden text-[10px] font-bold uppercase tracking-wider">
              {["active", "waiting", "closed"].map((v) => <button
    key={v}
    onClick={() => setQueueView(v)}
    className={"flex-1 py-1 " + (queueView === v ? "bg-brand/15 text-brand" : "bg-surface text-muted-foreground hover:text-foreground")}
  >
                  {v === "waiting" ? "Waiting" : v.charAt(0).toUpperCase() + v.slice(1)}
                </button>)}
            </div>
  ...
  >
              ...
              {playerQuery.trim() && <p className="mt-1 text-[9px] font-mono uppercase tracking-widest text-muted-foreground">
  ...
  >
          ...
          <div className="overflow-y-auto flex-1">
            {(() => {
    const myId = activeStaff?.id;
    const assignedToMe = visible.filter((t) => myId && t.assigneeId === myId);
    const allOther = visible.filter((t) => !(myId && t.assigneeId === myId));
    const renderTicket = (t) => {
      ...
      const typeShort = t.type === "player_report" && t.category ? REPORT_CATEGORY_LABEL[t.category] : TYPE_SHORT[t.type];
      const nameForRow = t.type === "player_report" ? t.subjectId ? getPlayer(t.subjectId).name : "\u2014" : getPlayer(t.reporterId).name;
      return <button
        key={t.id}
        onClick={() => setSelectedId(t.id)}
        style={{ borderLeftColor: TYPE_COLOR[t.type] }}
        className={"w-full text-left pl-2.5 pr-3 py-1.5 cursor-pointer transition-colors block border-l-[3px] " + (isActive ? "bg-surface/70" : "hover:bg-surface/30")}
      ...
      >
                    <div className="flex items-center gap-2 min-w-0">
                      <h3 className="text-xs font-medium truncate flex-1">
                        <span className="font-mono text-muted-foreground">{orgShort}</span>
                        <span className="text-muted-foreground/50 mx-1.5">·</span>
                        <span className="font-semibold" style={{ color: TYPE_COLOR[t.type] }}>{typeShort}</span>
                        <span className="text-muted-foreground/50 mx-1.5">·</span>
                        <span>{nameForRow}</span>
                      </h3>
                      {t.type === "player_report" && reportCount > 1 && <span className="text-[9px] font-mono text-muted-foreground shrink-0">
                          ×{reportCount}
                        </span>}
                      <span
      ...
      >
                      ...
                      {t.restrictedRank !== null && <Lock size={10} className="shrink-0 text-warning" />}
    ...
    return <>
                  ...
                  {allOther.length > 0 && <div>
                      {assignedToMe.length > 0 && <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm px-3 py-1.5 border-y border-border flex items-center justify-between">
                  ...
                  {visible.length === 0 && <div className="p-6 text-center text-xs text-muted-foreground">
    ...
    /* Detail */
        ...
        {selected ? <section className="flex-1 flex flex-col bg-surface/10 overflow-hidden min-w-0">

            <div className="px-6 py-4 border-b border-border flex flex-col gap-2">
              <div className="flex items-center gap-3 flex-wrap">

                <span
    className={"size-2.5 rounded-full shrink-0 " + (selected.status === "waiting_response" ? "bg-warning" : selected.status === "resolved" || selected.status === "closed" || selected.status === "cleared" || selected.status === "banned" ? "bg-danger" : "bg-success")}
  ...
  />
                ...
                <h1 className="text-lg font-semibold tracking-tight truncate">
                  {isReport ? `${selected.category ? REPORT_CATEGORY_LABEL[selected.category] : "Report"} \u2014 ${truncateName(subject?.name ?? "")}` : `${TICKET_TYPE_LABEL[selected.type]} \u2014 ${truncateName(reporter.name)}`}
                </h1>

                {selected.restrictedRank !== null && <span className="px-2 py-0.5 text-[10px] font-bold rounded ring-1 ring-warning/30 bg-warning/10 text-warning uppercase tracking-wider">
                    ◈ Hidden ≤ {selected.restrictedRank - 1}
                  </span>}
                <div className="flex gap-1.5 items-center flex-wrap ml-2">
                  <select
    value={selected.team}
    onChange={(e) => updateTicket(selected.id, { team: e.target.value })}
  ...
  >
    ...
    onChange={(e) => updateTicket(selected.id, { assigneeId: e.target.value || null })}
  ...
  >
                    ...
                    {!selected.assigneeId && activeStaff && <button
    onClick={assignToMe}
  ...
  >
                  ...
                  {canHide && <button
    onClick={toggleHide}
    className={"h-8 px-3 text-xs font-semibold rounded-md ring-1 transition-colors " + (isHidden ? "bg-warning/10 text-warning ring-warning/30 hover:bg-warning/20" : "bg-surface text-foreground ring-border hover:bg-surface-bright")}
  ...
  >
    ...
    className={"h-8 px-3 text-xs font-semibold rounded-md ring-1 transition-colors " + (proofOnly ? "bg-brand/15 text-brand ring-brand/30" : "bg-surface text-foreground ring-border hover:bg-surface-bright")}
  ...
  >
                    ...
                    <button
    onClick={selected.status === "cleared" ? reopenReport : markCleared}
    disabled={selected.status === "banned"}
    className={"h-8 px-3 text-xs font-semibold rounded-md ring-1 disabled:opacity-40 " + (selected.status === "cleared" ? "bg-warning/15 text-warning ring-warning/30 hover:bg-warning/25" : "bg-surface text-foreground ring-border hover:bg-surface-bright")}
  >
                      {selected.status === "cleared" ? "Unclose" : "Close"}
                    </button>
                    {selectedOrgRank >= 2 && <button
  ...
  >
                    ...
                    {selected.category === "toxicity" && <button
  ...
  >
                    ...
                    {selected.type === "ban_appeal" && <AppealModerationActions appellant={getPlayer(selected.reporterId)} />}
                    <button
  ...
  >
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              ...
              {!isReport && <div className="max-w-[60ch] bg-surface/40 ring-1 ring-border rounded-lg p-4 space-y-2">
              ...
              {isReport && selected.reports && <ReportsList
    reports={selected.reports}
    proofOnly={proofOnly}
    recencyDays={recencyDays}
  />}

              {[...selected.messages].map((m, i) => ({ m, i })).sort((a, b) => Number(!!a.m.pinned) - Number(!!b.m.pinned)).map(
    ({ m, i }) => m.authorKind === "system" ? <div
      key={i}
      className={"max-w-[60ch] ml-auto p-4 rounded-lg border " + (m.pinned ? "bg-warning/10 border-warning/30" : "bg-brand/5 border-brand/10")}
    >
                    <div className="flex items-center gap-2 mb-2">
                      <span
      className={"text-[10px] font-mono uppercase tracking-widest " + (m.pinned ? "text-warning" : "text-brand")}
    >
                        {m.pinned ? "\u{1F4CC} Pinned Note" : m.authorName === "System" ? "System Action" : "Internal Note"}
                      </span>
                      <span className={"h-px flex-1 " + (m.pinned ? "bg-warning/20" : "bg-brand/10")} />
                      <span className="text-[10px] text-muted-foreground font-mono">
                        {m.timestamp}
                      </span>
                      {m.authorName !== "System" && <button
      onClick={() => togglePin(i)}
      className={"text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ring-1 " + (m.pinned ? "text-warning ring-warning/30 hover:bg-warning/20" : "text-muted-foreground ring-border hover:text-foreground")}
      title={m.pinned ? "Unpin this note" : "Pin to bottom of thread"}
    >
                          {m.pinned ? "Unpin" : "Pin"}
                        </button>}
                    </div>
                    <p className="text-xs text-muted-foreground italic">{m.body}</p>
                  </div> : <div key={i} className="max-w-[60ch]">
                    <div className="flex items-start gap-4">
                      <div
      className={"size-8 rounded-sm shrink-0 grid place-items-center font-mono text-xs font-bold " + (m.authorKind === "staff" ? "bg-brand/20 text-brand" : "bg-surface-bright text-foreground")}
    >
                        {m.authorName.replace(/[\[\]]/g, "").slice(0, 2).toUpperCase()}
                      </div>
                      <div className="space-y-2 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {m.authorKind === "staff" && <span className="text-[9px] uppercase font-bold tracking-widest text-brand">
                              Staff
                            </span>}
                        </div>
                        <p className="text-sm text-foreground/90 leading-relaxed">{m.body}</p>
                      </div>
                    </div>
                  </div>
  )}
            ...
            {!isReport && <div className="p-6 border-t border-border bg-background">
                <div className="max-w-[60ch] mx-auto">
                  <div className="flex gap-2 mb-3">
                    <button
    onClick={() => setComposerMode("reply")}
    className={"text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded " + (composerMode === "reply" ? "bg-surface text-foreground" : "text-muted-foreground hover:text-foreground")}
  >
                      Reply
                    </button>
                    <button
    onClick={() => setComposerMode("note")}
    className={"text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded " + (composerMode === "note" ? "bg-brand/10 text-brand" : "text-muted-foreground hover:text-foreground")}
  >
                      Internal Note
                    </button>
                  </div>
                  {composerMode === "reply" && selected && <PredefineSearch
    orgId={orgForTicket(selected.number)}
    onPick={(content) => setDraft((d) => d ? `${d}${d.endsWith("\n") ? "" : "\n"}${content}` : content)}
  />}
                  <textarea
    value={draft}
    ...
    placeholder={composerMode === "reply" ? "Type your response to the player..." : "Leave an internal note for your team..."}
  />
                  <div className="flex items-center justify-between mt-2 gap-3">
                    {composerMode === "note" ? <label className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground cursor-pointer">
                        <input
    type="checkbox"
    checked={pinNote}
  ...
  />
                      ...
                      </label> : <label className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground cursor-pointer">
                        <input
    type="checkbox"
  ...
  >
          ...
          </section> : <section className="flex-1 grid place-items-center text-muted-foreground text-sm">
            No tickets match this filter.
          </section>}

        {selected && (selected.type === "player_report" ? <PlayerSidebar
    subject={subject}
    reporter={reporter}
  ...
  /> : <AppealSidebar ticket={selected} team={TEAM_META[selected.team].label} />)}
      </main>
    ...
    onSubmit={submitBan}
  />
          {selected.category === "toxicity" && <BanDialog
    open={muteDialogOpen}
  ...
  >
            ...
            <p className="text-sm text-muted-foreground mb-6">
              They wrote the most recent message and haven't received a staff reply. Do you want
              to give them a response before closing?
            </p>
...
function ReportsList({
  ...
  const byProof = proofOnly ? reports.filter((r) => r.evidence.trim().length > 0) : reports;
  const shown = cutoff > 0 ? byProof.filter((r) => {
    const t = Date.parse(r.submittedAt);
    return Number.isNaN(t) ? true : t >= cutoff;
  }) : byProof;
  const hiddenCount = reports.length - shown.length;
  const recencyLabel = recencyDays === 7 ? "past week" : recencyDays === 30 ? "past month" : recencyDays === 90 ? "past 3 months" : recencyDays === 365 ? "past year" : "all time";
  return <div className="max-w-[60ch] space-y-3">
    ...
    const tone = r.status === "banned" ? "text-danger ring-danger/30 bg-danger/10" : r.status === "case_closed" ? "text-muted-foreground ring-border bg-surface" : "text-warning ring-warning/30 bg-warning/10";
    ...
    >
                  {p.name.replace(/[\[\]]/g, "").slice(0, 2).toUpperCase()}
                </div>
    ...
    >
            ...
            <div>
              ...
              {hasEvidence ? <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-line font-mono break-all">
                  {r.evidence}
                </p> : <p className="text-xs text-muted-foreground italic">
                  No evidence attached.
                </p>}
            </div>
          </div>;
  })}
      {shown.length === 0 && <p className="text-xs text-muted-foreground italic">
          No reports with evidence yet. Reporters are still visible in the sidebar.
        </p>}
    </div>;
}
function Chip({
  ...
  const style = color ? active ? { backgroundColor: `color-mix(in oklab, ${color} 18%, transparent)`, color, boxShadow: `inset 0 0 0 1px color-mix(in oklab, ${color} 35%, transparent)` } : { color: `color-mix(in oklab, ${color} 75%, transparent)`, boxShadow: `inset 0 0 0 1px color-mix(in oklab, ${color} 25%, transparent)` } : void 0;
  return <button
    onClick={onClick}
    style={style}
    className={"px-2 py-1 text-[10px] font-semibold rounded uppercase transition-colors " + (color ? "hover:brightness-125" : active ? "bg-brand/10 text-brand ring-1 ring-brand/20" : "bg-surface text-muted-foreground ring-1 ring-border hover:text-foreground")}
  >
      {children}
    </button>;
}
function TypeBadge({ type }) {
  const tone = type === "player_report" ? "bg-danger/10 text-danger ring-danger/20" : type === "ban_appeal" ? "bg-warning/10 text-warning ring-warning/20" : type === "vip_issue" ? "bg-brand/10 text-brand ring-brand/20" : "bg-surface text-muted-foreground ring-border";
  return <span
...
function StatusBadge({ status }) {
  const tone = status === "banned" ? "bg-danger/10 text-danger ring-danger/20" : status === "cleared" ? "bg-success/10 text-success ring-success/20" : status === "waiting_response" ? "bg-warning/10 text-warning ring-warning/20" : status === "closed" ? "bg-surface text-muted-foreground ring-border" : "bg-surface text-muted-foreground ring-border";
  return <span
