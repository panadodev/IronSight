...
function DocsPage() {
  ...
  const [orgId, setOrgId] = useState(() => selectedOrgIds[0] ?? orgs[0]?.id ?? "");
  const effectiveOrgId = selectedOrgIds.includes(orgId) ? orgId : selectedOrgIds[0] ?? orgs[0]?.id ?? "";
  ...
  const orgArticles = useMemo(
    () => articles.filter(
      (a) => a.orgId === effectiveOrgId && myRank >= a.minRank
    ),
  ...
  const [creatingCatParent, setCreatingCatParent] = useState(
    void 0
  );
        ...
        <div className="w-72 border-r border-border bg-surface/30 flex flex-col">
          <div className="p-3 border-b border-border space-y-2">
            <div className="flex items-center justify-between">
              ...
              {selectedOrgIds.length > 1 && <Select value={effectiveOrgId} onValueChange={setOrgId}>
                  <SelectTrigger className="h-6 w-28 text-[10px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {orgs.filter((o) => selectedOrgIds.includes(o.id)).map((o) => <SelectItem key={o.id} value={o.id} className="text-xs">
                        {o.short}
                      </SelectItem>)}
                  </SelectContent>
                </Select>}
  ...
  /> : <Tree
    ...
    onRemoveCat={(id) => {
      if (confirm("Delete this category? Articles will be uncategorized.")) {
  ...
  />}
        ...
        <div className="flex-1 overflow-y-auto">
          {selected ? editing && editing.id === selected.id ? <Editor
    article={editing}
    cats={orgCats}
  ...
  /> : <div className="h-full grid place-items-center text-muted-foreground">
              <div className="text-center">
                ...
                <p className="text-sm">Pick an article from the left or search above.</p>
              </div>
            </div>}
        </div>
      </main>
...
function Tree({
  ...
  />)}
      {uncategorized.length > 0 && <div className="mt-3 pt-2 border-t border-border">
          <div className="px-2 text-[9px] font-mono uppercase tracking-widest text-muted-foreground mb-1">
            Uncategorized
          </div>
          {uncategorized.map((a) => <ArticleRow
    key={a.id}
    article={a}
    depth={0}
...
function CatNode({
  ...
  >
        ...
        {canEdit && <div className="hidden group-hover:flex items-center gap-0.5">
            <button
...
function ArticleRow({
  ...
  return <button
    ...
    className={"w-full flex items-center gap-1.5 px-1 py-1 rounded text-left text-xs " + (selected ? "bg-brand/15 text-foreground" : "text-muted-foreground hover:bg-surface hover:text-foreground")}
    style={{ paddingLeft: 14 + depth * 12 }}
  >
      <FileText className="size-3 shrink-0" />
      <span className="truncate flex-1">{article.title}</span>
      {article.minRank > 1 && <Lock className="size-3 text-muted-foreground shrink-0" />}
    </button>;
}
function SearchResults({
  ...
  if (results.length === 0) {
    return <p className="px-2 py-3 text-[11px] text-muted-foreground">No matches in articles you can view.</p>;
  }
  return <div className="space-y-0.5">
      {results.map((a) => <ArticleRow
    key={a.id}
    article={a}
    depth={0}
    selected={selectedId === a.id}
  ...
  return <article className="max-w-3xl mx-auto px-8 py-8">
      <header className="mb-6 pb-4 border-b border-border flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {canEdit && <Button size="sm" variant="outline" onClick={onEdit}>
              <Pencil className="size-3.5 mr-1" /> Edit
            </Button>}
          {canDelete && <Button size="sm" variant="ghost" onClick={onDelete} className="text-danger hover:text-danger">
...
`;
  ...
  return <div className="max-w-3xl mx-auto px-8 py-8 space-y-4">
  ...
  />
        ...
        <Button size="sm" onClick={() => onSave({ title, body, minRank, categoryId })}>
  ...
  >
            ...
            <SelectContent>
              ...
              {cats.map((c) => <SelectItem key={c.id} value={c.id}>
                  {catPath(c, cats)}
                </SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-1">
          ...
          <Select value={String(minRank)} onValueChange={(v) => setMinRank(Number(v))}>
            ...
            <SelectContent>
              {DOC_RANK_OPTIONS.map((o) => <SelectItem key={o.value} value={String(o.value)}>
                  {o.label}
                </SelectItem>)}
            </SelectContent>
          </Select>
      ...
      {preview ? <div className="ring-1 ring-border rounded-md p-4 bg-surface/30 min-h-[400px]">
  ...
  />}
      <p className="text-[10px] text-muted-foreground">
        Markdown supported (GFM). Paste images with Ctrl+V — they're embedded automatically. Ctrl+S to save.
      </p>
    </div>;
}
...
function NewCategoryDialog({
  ...
  >
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>{parentId ? "New subcategory" : "New category"}</DialogTitle>
...
function HistoryDialog({
  ...
  return <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Every save creates a snapshot. {canEdit ? "Sr. Admins and Management can restore." : "Read-only."}
        ...
        <div className="grid grid-cols-[260px_1fr] gap-4 max-h-[60vh]">
          <div className="overflow-y-auto border-r border-border pr-3 space-y-1">
            {article.versions.length === 0 && <p className="text-[11px] text-muted-foreground py-4">No previous versions yet.</p>}
            {article.versions.map((vv) => <div
    key={vv.id}
    className={"group p-2 rounded text-xs cursor-pointer ring-1 " + (previewing === vv.id ? "bg-brand/15 ring-brand/40" : "ring-border hover:bg-surface")}
  ...
  >
          ...
          <div className="overflow-y-auto">
            {v ? <MarkdownView body={v.body} /> : <p className="text-xs text-muted-foreground">Pick a version to preview.</p>}
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>
