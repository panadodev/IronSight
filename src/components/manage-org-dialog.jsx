...
function ManageOrgDialog({ open, onOpenChange }) {
  ...
  const effectiveOrgId =
    manageable.length === 1 ? manageable[0].id : selectedOrgId;
  ...
  const currentMembers = effectiveOrgId
    ? (orgMembers[effectiveOrgId] ?? [])
    : [];
  const orgName = effectiveOrgId
    ? orgs.find((o) => o.id === effectiveOrgId)?.name
    : null;
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {effectiveOrgId && sectionMeta
              ? `${orgName} \xB7 ${sectionMeta.label}`
              : effectiveOrgId
                ? `Manage ${orgName}`
          ...
          <DialogDescription>
            {effectiveOrgId && sectionMeta
              ? sectionMeta.blurb
              : effectiveOrgId
                ? "Pick what you want to configure for this organization."
                : "Pick which organization you want to manage."}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-3 py-1">
            ...
            <div className="grid grid-cols-2 gap-2">
              {SECTIONS.filter((s) => {
                ...
                if (rank < 4 && s.id !== "predefines" && s.id !== "toxicity")
                  return false;
                return true;
          ...
          <div className="space-y-4 py-1">
            ...
            {section === "predefines" && (
              <PredefinesPanel
                ...
                onUpdate={(id, patch) =>
                  updateOrgPredefine(effectiveOrgId, id, patch)
                }
            ...
            {section === "ban_reasons" && (
              <BanConfigsPanel
                ...
                muteConfig={
                  orgMuteConfigs[effectiveOrgId] ?? {
                    reasons: [],
                    noteFormat: "",
                  }
                }
                onAddReason={(cat, label) =>
                  addBanReason(effectiveOrgId, cat, label)
                }
                onUpdateReason={(cat, id, label) =>
                  updateBanReason(effectiveOrgId, cat, id, label)
                }
                onRemoveReason={(cat, id) =>
                  removeBanReason(effectiveOrgId, cat, id)
                }
                onSetNoteFormat={(cat, fmt) =>
                  setBanNoteFormat(effectiveOrgId, cat, fmt)
                }
                onAddMuteReason={(label) =>
                  addMuteReason(effectiveOrgId, label)
                }
                onUpdateMuteReason={(id, label) =>
                  updateMuteReason(effectiveOrgId, id, label)
                }
                onRemoveMuteReason={(id) =>
                  removeMuteReason(effectiveOrgId, id)
                }
                onSetMuteNoteFormat={(fmt) =>
                  setMuteNoteFormat(effectiveOrgId, fmt)
                }
              />
            )}
            ...
            {section === "toxicity" && (
              <ToxicityPanel
                ...
                onSave={(kind, phrases) =>
                  setOrgToxicityPhrases(effectiveOrgId, kind, phrases)
                }
              />
            )}
            ...
            {section === "tickets" && (
              <TicketTypesPanel
                enabled={
                  orgTicketTypes[effectiveOrgId] ??
                  TICKET_TYPE_KEYS.reduce(
                    (acc, k) => ({ ...acc, [k]: true }),
                    {},
                  )
                }
                onToggle={(key, value) =>
                  setOrgTicketTypeEnabled(effectiveOrgId, key, value)
                }
              />
            )}
                ...
                <div className="rounded-md ring-1 ring-border bg-surface/40 p-3 space-y-2">
                  ...
                  {!isOwner && newTeam === "management" && (
                    <p className="text-[11px] text-muted-foreground">
                      Heads-up: adding someone to Management gives them the same
                      powers as you.
                    </p>
                  )}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <Button
                            ...
                            disabled={isOwnerRow}
                            onClick={() =>
                              removeOrgMember(effectiveOrgId, m.staffId)
                            }
                            className="size-7"
                            title={
                              isOwnerRow
