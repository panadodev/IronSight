...
function PlayerCombobox({
  ...
  />
      {open && !disabled && <div className="absolute z-20 mt-1 w-full max-h-72 overflow-y-auto bg-background ring-1 ring-border rounded-md shadow-lg">
          {filtered.length === 0 ? <div className="p-3 text-xs text-muted-foreground">No matching players.</div> : filtered.map((p) => {
    ...
    return <button
      ...
      className={"w-full text-left px-3 py-2 flex items-center gap-3 transition-colors " + (active ? "bg-brand/10" : "hover:bg-surface/60")}
    >
                  <div
    ...
    >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={"text-sm font-medium truncate " + (active ? "text-brand" : "")}>
                      {p.name}
                    </p>
