...
import { Copy, UsersRound, Server as ServerIcon, X, Handshake, Flag, Trash2, RotateCcw } from "lucide-react";
import { fmtNum, getPlayer, getPreviousTeammates, getServerPlayers, getTeammates } from "@/lib/mock-data";
import { getAssociationsFor, addAssociationReports, hideAssociation, unhideAssociation, isAssociationHidden } from "@/lib/associations";
...
function CopyBtn({ text }) {
  ...
  >
      <Copy size={10} className={done ? "text-success" : "text-muted-foreground"} />
    </button>;
}
function Avatar({ player, size = 48 }) {
  ...
  >
      {player.name.replace(/[\[\]]/g, "").slice(0, 2).toUpperCase()}
    </div>;
}
function hash(s) {
  let h = 0;
...
function deriveStats(p) {
  ...
  const reasons = ["Toxicity", "Racism", "Cheating suspicion", "Team griefing", "Spam in chat", "Stream sniping"];
...
function PlayerSidebar({
  ...
  const visibleOffenses = d ? isSupportOnly ? d.offenses.filter((o) => o.type === "Mute") : d.offenses : [];
  void team;
  return <aside className="w-[28rem] shrink-0 border-l border-border bg-background overflow-y-auto">
      <div className="p-6 space-y-8">
        {subject && d && <section>
            <div className="bg-surface/60 ring-1 ring-border rounded-lg p-4">
              ...
              {!isSupportOnly && <div className="grid grid-cols-2 gap-y-3">
                {category === "cheating" ? <>
                    <Field label="S-Hours" value={fmtNum(subject.playtimeHours)} />
  ...
  />
                        <span className="text-[10px] text-muted-foreground">{d.pingMs}ms</span>
                      </p>
                    </div>
                  </> : category === "teaming" ? <>
                    <Field label="S-Hours" value={fmtNum(subject.playtimeHours)} />
  ...
  />
              ...
              {!isSupportOnly && category !== "cheating" && category !== "teaming" && <div className={category === "toxicity" ? "" : "mt-3 pt-3 border-t border-border"}>
        ...
        {subject && d && <section>
            ...
            {visibleOffenses.length === 0 ? <p className="text-xs text-muted-foreground italic">No prior offenses.</p> : <OffensesTable offenses={visibleOffenses} />}
          </section>}

        {subject && category === "cheating" && <ExternalBansSection subjectId={subject.steamId} />}

        {subject && category === "cheating" && !isSupportOnly && <LinkedAccountIntelSection
    subjectId={subject.steamId}
    subjectName={subject.name}
  />}

        {subject && category === "cheating" && <AlertsSection messages={messages ?? []} />}

        {subject && category === "cheating" && <HitDistanceSection subjectId={subject.steamId} />}


        {subject && category === "teaming" && <FriendlyRecipientsSection subjectId={subject.steamId} serverId={serverId ?? null} />}

        {subject && !isSupportOnly && category !== "toxicity" && <TeammatesSection subjectId={subject.steamId} category={category} />}

        {subject && category === "toxicity" && <ChatLogSection
    subjectId={subject.steamId}
    ticketStatus={ticketStatus}
    onAutoReopen={onAutoReopen}
  />}

        {subject && !isSupportOnly && category !== "teaming" && category !== "toxicity" && category !== "other" && <ServerHistorySection
    subjectId={subject.steamId}
  ...
  />}
        ...
        {reports ? <ReportersSection reports={reports} /> : <section>
        ...
        {subject && <PinnedPlayerNotesSection subjectId={subject.steamId} orgId={orgId} />}
      </div>
    </aside>;
}
function ReportersSection({ reports }) {
  ...
  return <section>
    ...
    const tone = r.status === "banned" ? "text-danger" : r.status === "case_closed" ? "text-muted-foreground" : "text-warning";
    return <li
      key={r.id}
...
function TeammatesSection({
  ...
  return <section className="space-y-4">
      {showCurrentBlock && <div>
        <h2 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-3 flex items-center justify-between">
          <span>{showPrevious ? "Current team" : "Team"} · {mates.length}</span>
        </h2>
        {mates.length === 0 ? <p className="text-xs text-muted-foreground italic">Solo player.</p> : <ul className="space-y-1.5">
            {mates.map((t) => <li
  ...
  >
      ...
      {showPreviousBlock && <div>
          <h2 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-3 flex items-center justify-between">
            <span>Previous team members · {previous.length}</span>
          </h2>
          {previous.length === 0 ? <p className="text-xs text-muted-foreground italic">No prior teammates on record.</p> : <ul className="space-y-1.5">
              {previous.map((t) => <li
    key={t.player.steamId}
    className="flex items-center gap-2 bg-surface/40 ring-1 ring-border rounded px-2 py-1"
  >
                  <span className="size-1.5 rounded-full shrink-0 bg-muted-foreground/50" title="Left team" />
                  <span className="text-[10px] font-medium truncate min-w-0">
                    {t.player.name}
                  </span>
                  <span className="text-[9px] font-mono text-muted-foreground flex items-center gap-1 shrink-0">
  ...
  return <>
      <div className="bg-surface/40 ring-1 ring-border rounded-lg overflow-hidden">
        <table className="w-full text-[10px] font-mono">
          ...
          <tbody>
            {offenses.map((o) => {
    const statusColor = o.statusTone === "danger" ? "text-danger" : o.statusTone === "warning" ? "text-warning" : "text-muted-foreground";
    return <tr key={o.id} className="border-t border-border">
                  <td className={`px-1.5 py-1 font-bold uppercase ${o.type === "Ban" ? "text-danger" : "text-warning"}`}>
                    {o.type}
                  </td>
    ...
    >
                  ...
                  <td className="px-1.5 py-1 text-right text-muted-foreground">{o.when}</td>
                </tr>;
  })}
          </tbody>
        </table>
  ...
  >
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold">
                <span className={open.type === "Ban" ? "text-danger" : "text-warning"}>
                  {open.type}
                </span>{" "}
                · {open.reason}
              </h3>
...
function AlertCategoryGroup({
  ...
  >
        <span className="text-xs text-muted-foreground flex items-center gap-1.5">
          <span
    className={"inline-block text-[8px] font-mono transition-transform " + (open ? "rotate-90" : "")}
  >
            ▶
          </span>
          {label}
  ...
  >
      ...
      {open && count > 0 && <div className="bg-background/40 border-t border-border">
          {items.map((m, i) => {
    const title = alertTitle(kind, m.body);
    const expanded = openItem === i;
    return <div key={i} className="border-b border-border last:border-0">
                <button
      type="button"
    ...
    >
                  <span
      className={"text-[8px] font-mono mt-1 transition-transform " + (expanded ? "rotate-90" : "")}
    >
                    ▶
                  </span>
                ...
                {expanded && <div className="px-3 pb-2 pt-1 text-[11px] font-mono whitespace-pre-wrap break-words text-foreground/90 bg-surface/40">
                    {m.body}
                  </div>}
              </div>;
  })}
...
function HitDistanceSection({ subjectId }) {
  ...
  const rows = buckets.map((b) => {
    ...
    const cells = parts.map((p) => {
      ...
      const tone = pct >= 60 ? "text-danger" : pct >= 40 ? "text-warning" : "text-foreground";
      return { ...p, pct, tone };
    });
  ...
  >
          ...
          <tbody>
            {rows.map((r) => <tr key={r.label} className="border-t border-border">
...
function buildServerSessions(subjectId, isOnline, windowDays = 7) {
  if (windowDays <= 7) {
    ...
    return Array.from({ length: count2 }).map((_, i) => {
      ...
      return { key: `${subjectId}-srv-${i}`, name, startHoursAgo, endHoursAgo, playedMin };
    });
  }
  ...
  return Array.from({ length: count }).map((_, i) => {
    ...
    const startHoursAgo = isOnline && i === 0 ? 0 : Math.floor(i * segment + jitter);
...
function ServerHistorySection({
  ...
  const servers = sessions.map((s) => {
    ...
    else if (s.startHoursAgo < 24) label = `${Math.round(s.startHoursAgo)}h ago`;
    else label = `${Math.floor(s.startHoursAgo / 24)}d ago`;
    const played = s.playedMin >= 60 ? `${Math.floor(s.playedMin / 60)}h ${s.playedMin % 60}m` : `${s.playedMin}m`;
    const overlaps = recipientSessions.filter(
      (r) => r.sessions.some(
        (rs) => rs.name === s.name && rs.startHoursAgo <= s.endHoursAgo && rs.endHoursAgo >= s.startHoursAgo
      )
    ).map((r) => r.name);
...
function getFriendlyRecipients(subjectId) {
  ...
  for (const e of events) {
    if (!map.has(e.target.steamId)) {
      map.set(e.target.steamId, { steamId: e.target.steamId, name: e.target.name });
    }
  }
  return Array.from(map.values());
}
const FRIENDLY_ACTIONS = ["Dropped items", "Received items", "Revived", "TC authed", "Turret authed"];
function buildFriendlyEvents(subjectId) {
  const mates = getTeammates(subjectId);
  const pool = mates.length > 0 ? mates.map((m) => m.player) : [
...
function FriendlyActionsSection({ subjectId }) {
  ...
  return <section>
      ...
      <div className="bg-surface/40 ring-1 ring-border rounded-lg overflow-hidden">
        <table className="w-full text-[10px] font-mono">
          <tbody>
            {visible.map((e) => <tr key={e.key} className="border-t border-border first:border-0">
                <td className="px-2 py-1 text-foreground whitespace-nowrap">{e.action}</td>
                <td className="px-2 py-1">
                  <button
    onClick={() => copy(e.target.steamId)}
    className="text-brand hover:underline truncate max-w-[110px] inline-block align-middle"
    title={copiedId === e.target.steamId ? `Copied ${e.target.steamId}` : `Copy ${e.target.steamId}`}
  >
                    {copiedId === e.target.steamId ? e.target.steamId : e.target.name}
                  </button>
                </td>
                <td className="px-2 py-1 text-right text-muted-foreground whitespace-nowrap">{e.when}</td>
              </tr>)}
          </tbody>
        </table>
      </div>
      {events.length > 5 && <button
    onClick={() => setOpen((v) => !v)}
    className="mt-2 text-[10px] font-mono uppercase tracking-widest text-brand hover:underline"
  >
...
function FriendlyRecipientsSection({
  subjectId,
  serverId
}) {
  ...
  const recipients = ranked.map((r, idx) => {
    ...
    if (idx === 0 && subjectSessions.length > 0) {
      ...
      if (subjectSessions90.length > 0) {
        const ref90 = subjectSessions90[Math.min(2, subjectSessions90.length - 1)];
        const shifted90 = {
          ...ref90,
    ...
    for (const s of subjectSessions) {
      for (const rs of sessions) {
        if (rs.name === s.name && rs.startHoursAgo <= s.endHoursAgo && rs.endHoursAgo >= s.startHoursAgo) {
          overlaps.push({
            server: s.name,
    ...
    let mutualFriends = recipientFriends.filter((f) => subjectFriendIds.has(f.steamId));
    if (idx === 0 && mutualFriends.length < 3) {
      const needed = STEAM_FRIEND_POOL.slice(0, 3).filter(
        (f) => !mutualFriends.some((m) => m.steamId === f.steamId)
      );
      mutualFriends = [...mutualFriends, ...needed].slice(0, Math.max(3, mutualFriends.length));
    }
    const reverseEvents = buildFriendlyEvents(r.steamId);
    let reciprocated = reverseEvents.some((e) => e.target.steamId === subjectId);
    if (idx === 0) reciprocated = true;
    const teamIds = new Set(getTeammates(subjectId).map((m) => m.player.steamId));
    ...
    const eventMinSec = r.events.length > 0 ? Math.min(...r.events.map((e) => e.secAgo)) : Number.POSITIVE_INFINITY;
    const assocSec = assocInfo ? Math.max(0, Math.floor((Date.now() - assocInfo.lastAt) / 1e3)) : Number.POSITIVE_INFINITY;
  ...
  function metaFor(target) {
    ...
    const subjectFriendIds = new Set(subjectFriends.map((f) => f.steamId));
    const mutualFriends = recipientFriends.filter((f) => subjectFriendIds.has(f.steamId));
    const reverseEvents = buildFriendlyEvents(target.steamId);
    const reciprocated = reverseEvents.some((e) => e.target.steamId === subjectId);
    const teamIds = new Set(getTeammates(subjectId).map((m) => m.player.steamId));
    ...
    const eventMinSec = evs.length > 0 ? Math.min(...evs.map((e) => e.secAgo)) : Number.POSITIVE_INFINITY;
    const assocSec = assocInfo ? Math.max(0, Math.floor((Date.now() - assocInfo.lastAt) / 1e3)) : Number.POSITIVE_INFINITY;
    return {
  ...
  let visible = ignoreTeamUi ? recipients.filter((r) => !r.inTeamUi) : recipients;
  visible = showHidden ? visible : visible.filter((r) => !r.hidden);
  visible = [...visible].sort(
    (a, b) => sortMode === "recent" ? a.assocLastSec - b.assocLastSec : b.associationReports - a.associationReports || b.signalCount - a.signalCount
  );
  let open = null;
  ...
  const openFromSearch = Boolean(open && !recipients.find((r) => r.steamId === open.steamId));
  const [searchQuery, setSearchQuery] = useState("");
  const serverPlayers = serverId ? getServerPlayers(serverId) : [];
  const trimmed = searchQuery.trim().toLowerCase();
  const matches = trimmed ? serverPlayers.filter((p) => p.steamId !== subjectId).filter(
    (p) => p.name.toLowerCase().includes(trimmed) || p.steamId.toLowerCase().includes(trimmed)
  ).slice(0, 8) : [];
  return <section>
    ...
    title={m === "most" ? "Sort by most associations" : "Sort by most recent association"}
  >
              {m === "most" ? "Most" : "Recent"}
            </button>)}
          <button
  ...
  >
      ...
      {serverId && <div className="relative mb-2">
          <input
    type="text"
    value={searchQuery}
  ...
  />
          {matches.length > 0 && <ul className="absolute z-10 left-0 right-0 mt-1 bg-background ring-1 ring-border rounded shadow-lg max-h-56 overflow-y-auto">
              {matches.map((p) => <li key={p.steamId}>
                  <button
    onClick={() => {
  ...
  >
                    ...
                    <span className="text-[9px] font-mono text-muted-foreground truncate">{p.steamId}</span>
                  </button>
                </li>)}
            </ul>}
          {trimmed && matches.length === 0 && <p className="absolute z-10 left-0 right-0 mt-1 bg-background ring-1 ring-border rounded p-2 text-[10px] text-muted-foreground italic">
  ...
  >
    ...
    title={r.associationReports > 0 ? `Last reported as associated ${secAgoLabel(r.assocLastSec)} \xB7 ${r.associationReports} report${r.associationReports === 1 ? "" : "s"}` : `Last association signal ${secAgoLabel(r.lastActivitySec)}`}
  >
              {secAgoLabel(
  ...
  >
    ...
    onAddAssociated={openFromSearch && !open.hidden ? () => {
...
function RecipientBreakdownModal({
  ...
  for (const e of filteredEvents) breakdown.set(e.action, (breakdown.get(e.action) ?? 0) + 1);
  const subjSessions = serverWin.days === 7 ? subjectSessions : subjectSessions90;
  const recipSessions = serverWin.days === 7 ? recipient.sessions : recipient.sessions90;
  const serverNames = Array.from(
    /* @__PURE__ */ new Set([...subjSessions.map((s) => s.name), ...recipSessions.map((s) => s.name)])
  );
  const rows = serverNames.map((name) => {
    ...
    const hasOverlap = subj.some(
      (a) => recip.some(
        (b) => a.startHoursAgo <= b.endHoursAgo && a.endHoursAgo >= b.startHoursAgo
      )
    );
    return { name, subj, recip, hasOverlap };
  }).sort((a, b) => Number(b.hasOverlap) - Number(a.hasOverlap));
  const maxHours = serverWin.days * 24;
  const xPct = (hoursAgo) => Math.max(0, Math.min(100, (maxHours - hoursAgo) / maxHours * 100));
  return <div
    className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"
    onClick={onClose}
  >
        ...
        <div className="flex items-start justify-between mb-4 gap-2">
          <div className="min-w-0">
            <h3 className="text-sm font-semibold flex items-center gap-1.5">
              {recipient.name}
              {recipient.steamFriend && <UsersRound size={12} className="text-success" />}
              {recipient.overlaps.length > 0 && <ServerIcon size={12} className="text-warning" />}
            </h3>
  ...
  >
        ...
        {recipient.hidden && onReassociate && <div className="mb-4 px-3 py-2 rounded ring-1 ring-amber-400/30 bg-amber-400/5 flex items-center gap-3">
            <Trash2 size={12} className="text-amber-400 shrink-0" />
            <p className="text-[11px] text-foreground flex-1">
              These players were previously associated but were manually unassociated.
            </p>
            <button
  ...
  >
        ...
        {canAddAssociated && onAddAssociated && <div className="mb-4 px-3 py-2 rounded ring-1 ring-brand/30 bg-brand/5 flex items-center gap-3">
            <p className="text-[11px] text-foreground flex-1">
  ...
  >
        ...
        {recipient.steamFriend && <div className="mb-3 px-3 py-2 rounded ring-1 ring-success/30 bg-success/5 flex items-center gap-2">
            <UsersRound size={12} className="text-success shrink-0" />
            <p className="text-[11px] text-foreground">
              <span className="font-semibold text-success">Friends on Steam</span> — the
              subject and {recipient.name} are connected on Steam.
            </p>
          </div>}


        {recipient.associationReports > 0 && <div className="mb-4 px-3 py-2 rounded ring-1 ring-rose-400/30 bg-rose-400/5 flex items-center gap-2">
            ...
            <p className="text-[11px] text-foreground">
              ...
              {recipient.associationReports === 1 ? "time" : "times"} via the public
              report flow.
            </p>
          </div>}
  ...
  >
        ...
        {breakdown.size > 0 && <div className="flex flex-wrap gap-1.5 mb-2">
            {Array.from(breakdown.entries()).sort((a, b) => b[1] - a[1]).map(([action, count]) => <span
    key={action}
  ...
  >
        ...
        <div className="bg-surface/40 ring-1 ring-border rounded mb-5 overflow-hidden max-h-48 overflow-y-auto">
          {filteredEvents.length === 0 ? <p className="text-[10px] text-muted-foreground italic p-2">
              No actions in this range.
            </p> : <table className="w-full text-[10px] font-mono">
              <tbody>
                {filteredEvents.map((e, i) => <tr
  ...
  >
        ...
        <div className="space-y-2">
          {rows.map((row) => <div key={row.name}>
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {row.hasOverlap && <span className="text-[9px] font-mono text-warning font-bold uppercase tracking-wider">
                    overlap
                  </span>}
              </div>
    ...
    /* day gridlines */
                ...
                {Array.from({ length: serverWin.days === 7 ? 7 : 9 }).map((_, i) => {
  ...
  />)}
    ...
    /* MUTUAL STEAM FRIENDS */
        ...
        <div className="mt-5">
          ...
          {recipient.mutualFriends.length === 0 ? <p className="text-[10px] text-muted-foreground italic">
              No friends in common on Steam.
            </p> : <ul className="space-y-1">
              {recipient.mutualFriends.map((f) => <li
    key={f.steamId}
    className="flex items-center gap-2 bg-surface/40 ring-1 ring-border rounded px-2 py-1"
  >
...
function FriendlyNotes({
  ...
  >
      ...
      {notes.length === 0 ? <p className="text-[10px] text-muted-foreground italic">
...
function buildChatLog(subjectId) {
  ...
  const times = ["14:22", "14:18", "14:05", "13:51", "13:40", "13:22", "12:58", "12:31", "11:47", "11:12", "10:38", "09:55"];
  const out = [];
  const count = 8 + h % 5;
...
function ChatLogSection({
  ...
  const classified = log.map((l) => ({ ...l, flag: classifyLine(l.body, yellow, red) }));
  ...
  useEffect(() => {
    ...
    onAutoReopen(`Auto-reopened: red phrase detected in chat \u2014 "${firstRed.body}"`);
  }, [onAutoReopen, ticketStatus, firstRed, subjectId]);
  return <section>
      <h2 className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground mb-3 flex items-center justify-between">
        <span>Chat Log · {log.length}</span>
        {flaggedCount > 0 && <span className="font-mono normal-case tracking-normal text-danger">
            {flaggedCount} flagged
          </span>}
      </h2>
      <div className="space-y-1.5 max-h-96 overflow-y-auto pr-1">
        {classified.map((line, i) => {
    const bgRing = line.flag === "red" ? "bg-danger/10 border border-danger/30" : line.flag === "yellow" ? "bg-warning/10 border border-warning/30" : "bg-muted/30 border border-border";
    const bodyColor = line.flag === "red" ? "text-danger" : line.flag === "yellow" ? "text-warning" : "text-foreground";
    return <div
      key={i}
    ...
    >
              ...
              <span className={`flex-1 break-words ${bodyColor}`}>{line.body}</span>
            </div>;
  })}
      </div>
    </section>;
}
const KF_WEAPONS = ["AK-47", "Bolt Action", "MP5", "Custom SMG", "Semi Rifle", "DB Shotgun", "Pump Shotgun", "M249", "L96", "Revolver", "Python", "Compound Bow", "Crossbow", "Eoka", "Salvaged Sword", "Rock"];
const KF_PVE = ["Wolf", "Bear", "Boar", "Scientist", "Heavy Scientist", "Patrol Helicopter", "Bradley APC", "Fall damage", "Drowning", "Cold", "Hunger", "Barbed wire", "Landmine", "Bear trap"];
const KF_SUICIDES = ["Suicide (F1)", "Disconnected (bleeding)", "Wounded out"];
const KF_OPPONENTS = ["RAGE_KING", "moose.exe", "Doc_Holiday", "stonk", "v0idwalker", "kreepyKev", "ZyklonZ", "sunshine.gg", "MrTuna", "frostbyte", "neonNomad", "ghostFace", "cR4cked", "saltyDog", "Mr. Crab", "PixelPanda", "rustler44", "scrapHoarder", "tarpRoof", "wood1k", "Beanieboi", "M3lon"];
function buildKillFeed(subjectId) {
  let h = 0;
  for (let i = 0; i < subjectId.length; i++) h = h * 33 + subjectId.charCodeAt(i) | 0;
  h = Math.abs(h) || 1;
...
function KillFeedSection({ subjectId }) {
  ...
  const deaths = feed.filter((e) => e.kind === "death" || e.kind === "pve" || e.kind === "suicide").length;
  ...
  return <section>
      ...
      <div className="space-y-1.5 max-h-96 overflow-y-auto pr-1">
        {feed.map((e, i) => {
    const ringBg = e.kind === "kill" ? "bg-success/10 border border-success/30" : e.kind === "suicide" ? "bg-warning/10 border border-warning/30" : "bg-danger/10 border border-danger/30";
    const tag = e.kind === "kill" ? { label: "KILL", color: "text-success" } : e.kind === "death" ? { label: "DIED", color: "text-danger" } : e.kind === "pve" ? { label: "PVE", color: "text-danger" } : { label: "SELF", color: "text-warning" };
    return <div
      key={i}
    ...
    >
              ...
              <span className="flex-1 break-words text-foreground">
                {e.kind === "kill" && <>
                    killed <span className="font-mono text-foreground">{e.other}</span>{" "}
                    <span className="text-muted-foreground">
                      · {e.weapon} · {e.distance}m
                    </span>
                    {e.headshot && <span className="ml-1 font-mono text-[9px] uppercase text-warning">
                        headshot
                      </span>}
                  </>}
                {e.kind === "death" && <>
                    killed by <span className="font-mono text-foreground">{e.other}</span>{" "}
                    <span className="text-muted-foreground">
                      · {e.weapon} · {e.distance}m
                    </span>
                    {e.headshot && <span className="ml-1 font-mono text-[9px] uppercase text-warning">
                ...
                {e.kind === "pve" && <>
                    died to <span className="font-mono text-foreground">{e.other}</span>
                  </>}
                {e.kind === "suicide" && <span className="font-mono text-foreground">{e.other}</span>}
              </span>
            </div>;
  })}
      </div>
