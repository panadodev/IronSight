...
function PredefineSearch({
  ...
  >
      ...
      {open && <div className="absolute z-20 left-0 right-0 bottom-full mb-1 max-h-[260px] overflow-y-auto bg-background border border-border rounded-md shadow-lg">
          {matches.length === 0 ? <p className="text-[11px] text-muted-foreground italic p-3">
              {items.length === 0 ? "No pre-defines yet for this org. Add some in Manage Org \u2192 Pre-defines." : "No pre-defines match your search."}
            </p> : <ul className="divide-y divide-border">
              {matches.map((p) => <li key={p.id}>
                  <button
    type="button"
    onMouseDown={(e) => {
      e.preventDefault();
  ...
  >
                    <p className="text-xs font-semibold font-mono text-brand">{p.keyword}</p>
