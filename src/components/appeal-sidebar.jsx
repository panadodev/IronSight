...
function statusText(o, ov) {
  if (ov?.lifted) return { label: `Lifted via appeal \xB7 ${ov.at}`, tone: "success" };
...
function ManageDialog({
  ...
  const list = useMemo(
    () => offenses.filter((o) => o.type === kind).sort((a, b) => Number(b.active) - Number(a.active)),
    [offenses, kind]
  );
  const now = "just now";
  const apply = (id) => {
  ...
  const lift = (id) => setOverrides((p) => ({ ...p, [id]: { lifted: true, at: now } }));
  return <Dialog>
      ...
      <DialogContent className="max-w-xl">
        ...
        {list.length === 0 ? <p className="text-xs text-muted-foreground italic py-6 text-center">
            No {kind.toLowerCase()}s on record.
          </p> : <ul className="space-y-2 max-h-[60vh] overflow-y-auto">
            {list.map((o) => {
    const ov = overrides[o.id];
    const st = statusText(o, ov);
    const editable = o.active && !ov?.lifted;
    return <li key={o.id} className="bg-surface/40 ring-1 ring-border rounded p-3 space-y-2">
    ...
    >
                  ...
                  {editable && <div className="flex items-center gap-2">
                      <Select
      value={drafts[o.id] ?? ""}
      onValueChange={(v) => setDrafts((p) => ({ ...p, [o.id]: v }))}
    >
                        <SelectTrigger className="h-8 text-xs flex-1">
                          <SelectValue placeholder="New duration" />
                        </SelectTrigger>
                        <SelectContent>
                          {LENGTH_OPTIONS.map((opt) => <SelectItem key={opt.id} value={opt.id} className="text-xs">
                              {opt.label}
                            </SelectItem>)}
                        </SelectContent>
                      </Select>
...
function AppealModerationActions({ appellant }) {
  ...
  const activeBans = offenses.filter((o) => o.type === "Ban" && o.active).length;
  const activeMutes = offenses.filter((o) => o.type === "Mute" && o.active).length;
  const btnClass = "inline-flex items-center gap-1.5 h-8 px-3 bg-surface text-foreground text-xs font-semibold rounded-md ring-1 ring-border hover:bg-surface-bright";
  return <>
      <ManageDialog
    kind="Ban"
    offenses={offenses}
    trigger={<button type="button" className={btnClass}>
            <Ban className="size-3.5" />
            Manage Bans
            {activeBans > 0 && <span className="font-mono text-[10px] text-muted-foreground">{activeBans}</span>}
          </button>}
  />
      <ManageDialog
    kind="Mute"
    ...
    trigger={<button type="button" className={btnClass}>
            ...
            Manage Mutes
            {activeMutes > 0 && <span className="font-mono text-[10px] text-muted-foreground">{activeMutes}</span>}
          </button>}
  />
    </>;
}
...
function buildPermissions(steamId) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let g = 0; g < groupCount; g++) {
      const cand = OXIDE_GROUPS[1 + (seed + g * 7) % (OXIDE_GROUPS.length - 1)];
...
function AppealSidebar({ ticket, team }) {
  ...
  return <aside className="w-[28rem] shrink-0 border-l border-border bg-background overflow-y-auto">
      <div className="p-6 space-y-8">
        {isVip ? <section>
            <div className="bg-surface/60 ring-1 ring-border rounded-lg p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h3 className="text-sm font-semibold truncate">{appellant.name}</h3>
        ...
        {(isSupport || isAppeal) && stats ? <section>
            <div className="bg-surface/60 ring-1 ring-border rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-y-3">
                <Field label="S-Hours" value={fmtNum(appellant.playtimeHours)} />
                <Field label="BM-Hours" value={fmtNum(stats.bmHours)} />
                <Field label="AT-Hours" value={fmtNum(stats.atHours)} />
                {isAppeal ? <Field label="K.D" value={stats.kd.toFixed(2)} /> : null}
                {isAppeal ? <Field label="Hit %" value={`${stats.hitPct}%`} /> : null}
                <Field
    label="Proxy"
  ...
  />
                    <span className="text-[10px] text-muted-foreground">{stats.pingMs}ms</span>
                  </p>
                </div>
              </div>
            </div>
        ...
        {!isVip && !isSupport && !isAppeal && <section>
    ...
    /* Moderation actions moved to ticket header */
        ...
        {(isSupport || isAppeal) && stats && <section>
            ...
            {stats.offenses.length === 0 ? <p className="text-xs text-muted-foreground italic">No prior offenses.</p> : <OffensesTable offenses={stats.offenses} />}
          </section>}
        ...
        {isAppeal && <FriendlyRecipientsSection
    subjectId={appellant.steamId}
    serverId={ticket.serverId ?? null}
  />}

        {(isSupport || isAppeal) && <TeammatesSection
    subjectId={appellant.steamId}
    category={isAppeal ? "teaming" : void 0}
  />}

        {isAppeal && <ChatLogSection subjectId={appellant.steamId} ticketStatus={ticket.status} />}

        {(isSupport || isAppeal) && <ServerHistorySection
    subjectId={appellant.steamId}
...
function Row({ label, value, mono }) {
  return <div className="flex justify-between text-xs py-1.5 border-b border-border">
