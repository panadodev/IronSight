...
export function NewBanDialog({
  ...
  const canOfferIpBan =
    canIssueIp && (!playerSteamId || ipBanEligibility.allowed);
          ...
          {rconResults ? (
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                ...
                {orgServers.length > 0 ? (
                  ...
                  <p className="text-xs text-muted-foreground">
                    No servers registered for this org. The ban will be saved
                    but not pushed via RCON.
                  </p>
                )}
              </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            setLinkedMediaIds((ids) =>
                              ids.filter((id) => id !== item.mediaId),
                            );
                            setLinkedMediaItems((items) =>
                              items.filter((i) => i.mediaId !== item.mediaId),
                            );
                          }}
              ...
              <DialogFooter>
                ...
                <Button
                  onClick={submit}
                  disabled={submitting || ipSubmitBlocked}
                >
                  {submitting
                    ? "Issuing…"
                    : `Issue ${actionType === "mute" ? "Mute" : "Ban"}`}
